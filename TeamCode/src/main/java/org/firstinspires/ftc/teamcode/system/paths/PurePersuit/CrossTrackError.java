...
public class CrossTrackError
{
    ...
    public static void calculateCTE(Point robotPosition, Point lookAheadPoint, double radius)
    {
        double a = - Math.tan(worldAngle_rad);
        double b = 1;
        double c = Math.tan(worldAngle_rad) * robotPosition.x - robotPosition.y;

        double d = Math.abs(a * lookAheadPoint.x + b * lookAheadPoint.y + c) / Math.hypot(robotPosition.x, robotPosition.y);
        ...
        public double calculateLookahead(double CTE, double minLookahead, double maxLookahead, double maxCTE)
        {
            // Normalize the CTE
            double normalizedCTE = Math.min(CTE / maxCTE, 1);

            // Calculate lookahead (linear relationship)
            return maxLookahead - (normalizedCTE * (maxLookahead - minLookahead));
        }
