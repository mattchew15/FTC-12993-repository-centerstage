...
import static org.firstinspires.ftc.teamcode.system.hardware.OuttakeSubsystem.MINI_TURRET_BACKPURPLE_POS;
import static org.firstinspires.ftc.teamcode.system.hardware.OuttakeSubsystem.MINI_TURRET_FRONTPURPLE_POS;
import static org.firstinspires.ftc.teamcode.system.hardware.OuttakeSubsystem.MINI_TURRET_STRAIGHT_POS;
...
public class AutoSequences {
    ...
    public AutoSequences(Telemetry telemetry, int trussMiddleStage){
        ...
        autoTrajectories = new AutoTrajectories(trussMiddleStage, telemetry); // road drive class
    ...
    public boolean preloadDriveState (boolean railLeftOrRight, boolean preDropPurple, double railDelay, double slideExtendSpeed, boolean preExtendSlides){
        ...
        //extendIntakeArms();
        //intakeSubsystem
        outtakeSubsystem.liftToInternalPID(trussMiddleStage == 3? 1.2:3.9,1); // so thing doesn't hit
        ...
        if (trussMiddleStage == 3? Math.abs(yPosition) < 47: Math.abs(yPosition) < 45 ){
            ...
            if (railLeftOrRight){
                if (S != 1){
                    outtakeSubsystem.setOuttakeRailServo(RAIL_CENTER_POS - purple1Position);

                }  else {
                    outtakeSubsystem.setOuttakeRailServo(RAIL_CENTER_POS + purple1Position);
                }
               // S != 1? outtakeSubsystem.setOuttakeRailServo(RAIL_CENTER_POS - 0.27): outtakeSubsystem.setOuttakeRailServo(RAIL_CENTER_POS + 0.27);
            } else {
                if (S != 1){
                    outtakeSubsystem.setOuttakeRailServo(RAIL_CENTER_POS + purple1Position);
            ...
            if (trussMiddleStage == 3? Math.abs(yPosition) < 40: Math.abs(yPosition) < 45){
                if (teamPropLocation == 1){
                    if(railLeftOrRight){
                        if (S == 1) outtakeSubsystem.miniTurretState(OuttakeSubsystem.MiniTurretState.FRONT_PURPLE);
                        if (S == -1) outtakeSubsystem.setMiniTurretServo(MINI_TURRET_STRAIGHT_POS - 0.095);
                    } else {
                        if (S == 1){
                    ...
                    if(railLeftOrRight){
                        if (S == 1) outtakeSubsystem.miniTurretState(OuttakeSubsystem.MiniTurretState.BACK_PURPLE);
                        if (S == -1) outtakeSubsystem.setMiniTurretServo(MINI_TURRET_STRAIGHT_POS + 0.069);
    ...
    public boolean preloadDriveState3 (){
        ...
        if (Math.abs(yPosition) < y1){
            outtakeSubsystem.armServoState(OuttakeSubsystem.ArmServoState.SCORE_PURPLE);
            if (Math.abs(yPosition) < y2){
                outtakeSubsystem.setOuttakePitchPurplePixelThirdCasePosition();

                intakeSubsystem.intakeSlideInternalPID(intakeSlidePosition,0.7);
                intakeSubsystem.intakeSpin(1);
                intakeSubsystem.intakeArmServoState(IntakeSubsystem.IntakeArmServoState.VERY_TOP);
            }
        }
    ...
    public boolean afterPreloadDriveState3(){
        ...
        outtakeSubsystem.miniTurretState(OuttakeSubsystem.MiniTurretState.STRAIGHT);
        if (teamPropLocation == 3? delay(180): delay(450)){
            outtakeSubsystem.armServoState(OuttakeSubsystem.ArmServoState.READY);
    ...
    public boolean transferPixel(boolean gripEarly){
        ...
        if ( (numCycles == 0? teamPropLocation != 1? delay(400): delay(850): delay(310)) && (trussMiddleStage == 1? intakeSubsystem.intakeSlidePosition < 530: intakeSubsystem.intakeSlidePosition < 595)){ // time for pixel holder to close/reverse
            ...
            if ((intakeSubsystem.intakeSlidePosition < 23) && ticksToInchesSlidesMotor(outtakeSubsystem.liftPosition) < 0.13){
               // if (numCycles == 0? delay(1600):true){
                    ...
                    if(intakeSubsystem.backColourSensorValue > 700) {
                        extendOuttake = true;
                    }
                    else {
                        extendOuttake = false;
                    }
                    if (numCycles == 0 || trussMiddleStage == 2){ // so that it will always extend first cycle
                        extendOuttake = true; // always extend on the middle auto
                    }
...
*/
            ...
            if (numCycles == 0 && !autoTrajectories.extendSlidesAroundTruss? delay(500):true){
                if (extendOuttake){
                    ...
                    if (numCycles == 0 && differentPitchForYellowRail){ // consistent across all autos
                        ...
                        outtakeSubsystem.outtakePitchServoKeepToPitch(outtakeSubsystem.pitchEncoderPosition);
                        outtakeSubsystem.armServoState(OuttakeSubsystem.ArmServoState.SCORE_AUTO); // slides go out before arm so transfer is good
                    }
                }
                ...
                if (delay(160)){ // time for grippers to close
                    ...
                    if (extendOuttake){
                        ...
                        if (ticksToInchesSlidesMotor(outtakeSubsystem.liftPosition) > 2){
                    ...
                    if (delay(365)){ // once chute is down
                        if (extendOuttake){
                            outtakeSubsystem.miniTurretPointToBackdrop(correctedHeading);
                            if(extendRailLate){
                                if (xPosition > 14.7){ //(trussMiddleStage == 3 && numCycles ==0)? xPosition > 31:
                                    autoRail.railLogic();
                                }
                            } else {
                                autoRail.railLogic();
    ...
    public boolean grabOffStackTruss(double delayBeforeRetracting){
        ...
        if (autoTrajectories.extendSlidesAroundTruss){ // not for the last case??
            ...
            intakeSlideToSlowedEnd(numCycles == 1?800:numCycles == 2?815:1000,110,0.2,1);
         // for some reason we can't extend the slides unless we are further in
            intakeSubsystem.intakeSpin(1);
            if (!autoTrajectories.drive.isBusy() && intakeSubsystem.intakeSlideTargetReached() || intakeSubsystem.pixelsInIntake()){ // || xPosition < -27 && intakeSubsystem.pixelsInIntake()
                resetTimer();
                autoTrajectories.extendSlidesAroundTruss = false;
                autoTrajectories.extendSlidesAroundStage = false;
                return true;
    ...
    public void idle(){
        ...
        if ( ticksToInchesSlidesMotor( outtakeSubsystem.liftPosition) < 15 ){
            outtakeSubsystem.outtakeRailState(OuttakeSubsystem.OuttakeRailState.CENTER);
        } // else {
    ...
    public void preExtendIntakeSlides(double slideExtendSpeed, int slideOffset){
        if (teamPropLocation == 1 || frontThirdCase){
            intakeSlideToSlowedEnd(100+slideOffset,40,0.2,1);
...
//                intakeSubsystem.intakeSlideTo(slideToPosition,intakeSubsystem.intakeSlidePosition,1);
        ...
        if ((delay(280)) && timesIntoStack < 1 && numCycles != 0){ // go back into stack formore pixels case
            if (!intakeSubsystem.pixelsInIntake()){
                ...
                if (trussMiddleStage == 3 && numCycles == 1){
                    armHeight = 3;
                } else if(trussMiddleStage == 3 && numCycles >= 3) {
                    armHeight = 1;
                }
                intakeSubsystem.intakeChuteArmServoState(IntakeSubsystem.IntakeChuteServoState.READY);
                intakeSubsystem.intakePixelHolderServoState(IntakeSubsystem.IntakePixelHolderServoState.OPEN);
                timesIntoStack += 1;
                resetTimer();
                autoTrajectories.goBackToStack(poseEstimate,slowerStackVelocity,trussMiddleStage,xPosition, headingPosition);
    ...
    public void setPlaceForBack(){
        if (S ==1){
            if (teamPropLocation == 1 ){
                place = Place.LEFT;
            } else if (teamPropLocation == 3 || teamPropLocation == 2){
                place = Place.RIGHT;
            }
        } else {
            if (teamPropLocation == 1){
