...
public class Front_RED_MIDDLE_CYCLE extends LinearOpMode {
    ...
    // same cycle numbers
    AutoRail railLogic = new AutoRail(numCycleForDifferentLane,0, auto);
...
currentState != AutoState.PRELOAD_DRIVE && currentState != AutoState.OUTTAKE_PIXEL);
    ...
    public void autoSequence(){
        ...
        switch (currentState) {
            case DELAY:
                if(auto.delayState(0,telemetry)){
            ...
            case OUTTAKE_PIXEL:
                ...
                if (numCycles == 0){ // for very first cycle
                    ...
                    pitchTarget = 26;
                    liftTarget = 30;
                    intakeTrajectory = auto.autoTrajectories.driveIntoStackAngledAfterAngledOuttakeTrajectory(poseEstimate,12,3,-170);
                } else if (numCycles == 6){
            ...
            case GRAB_OFF_STACK:
                ...
                if (numCycles > 4){
                    outtakeTrajectory = auto.autoTrajectories.outtakeDriveFromAngleTurnEndTrajectory(poseEstimate,16, -5, 4);
                }
                else if (numCycles > 2) {
                    outtakeTrajectory = auto.autoTrajectories.outtakeDriveFromStraightTurnEndStageV3Trajectory(poseEstimate,16, -5, 4);
                } else {
                    outtakeTrajectory = auto.autoTrajectories.outtakeDriveMiddlePathTrajectory(poseEstimate,16, 26, MiddleLaneYDeposit);
                }

                boolean grabOffStackFinished = auto.grabOffStack(numCycleForDifferentLane, true, outtakeTrajectory);
                if (grabOffStackFinished){
                    currentState = AutoState.AFTER_GRAB_OFF_STACK;
                }
                break;
            case AFTER_GRAB_OFF_STACK:
                 if (auto.afterGrabOffStack(2,3)){
                     currentState = AutoState.TRANSFER_PIXEL;
                 }
                break;
            case PARK:
