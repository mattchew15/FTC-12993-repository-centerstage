...
import static Main.src.RobotUtilities.MovementVars.movement_turn;
import static Main.src.RobotUtilities.MovementVars.movement_x;
import static Main.src.RobotUtilities.MovementVars.movement_y;
import static Main.src.com.company.Robot.worldAngle_rad;
import static Main.src.com.company.Robot.worldXPosition;
import static Main.src.com.company.Robot.worldYPosition;
 ...
 */
    public static double[] powerFromVector()
    {
        ...
        double leftF = Range.clip(power * cos / max + movement_turn, -1, 1);
        double rightF = Range.clip(power * sin / max - movement_turn, -1, 1);
        double leftB = Range.clip(power * cos / max + movement_turn, -1, 1);
        double rightB = Range.clip(power * sin / max - movement_turn, -1, 1);
    ...
    public static double[] powerPID()
    {
        ...
        double leftF = Range.clip(xRotated + yRotated + t, -1, 1);
        double rightF = Range.clip(xRotated - yRotated - t, -1, 1);
        double leftB = Range.clip(xRotated - yRotated + t, -1, 1);
        double rightB = Range.clip(xRotated + yRotated - t, -1, 1);
